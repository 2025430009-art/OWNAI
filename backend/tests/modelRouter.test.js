...
import { detectTask, INTELLIGENCE_MODES, OLLAMA_MODELS } from '../src/utils/modelRouter.js';

describe('modelRouter', () => {
  it('routes code tasks to DEEP mode with qwen', () => {
    const result = detectTask('fix this javascript bug in my function');
    assert.equal(result.mode, INTELLIGENCE_MODES.DEEP);
    assert.equal(result.model, OLLAMA_MODELS.CODE);
  });

  it('routes reasoning tasks to THINK mode with fast fallback model', () => {
    const result = detectTask('why does this happen? analyze the issue');
    assert.equal(result.mode, INTELLIGENCE_MODES.THINK);
    assert.equal(result.model, OLLAMA_MODELS.FALLBACK);
  });
  ...
  it('defaults to FAST mode with llama3.2:3b', () => {
    const result = detectTask('hello');
    assert.equal(result.mode, INTELLIGENCE_MODES.FAST);
    assert.equal(result.model, OLLAMA_MODELS.CHAT);
  });
});
